...
**D3 — el `catch` externo de `/compose` tapa la causa real (hallazgo de la verificación en ejecución del 11-sep, confirmado en producción a las 16:25).** Con `wrangler dev` y llaves falsas, `/compose` con 1 foto devolvió `502 «Scene service unavailable. Submission may have been accepted; do not retry automatically.»` cuando la causa era `upload-url 401: Invalid credentials` — la subida falló ANTES de enviar nada a fal, así que «may have been accepted» es falso. En producción, la primera escena real por fal falló a los 43 s con ese mismo texto y no sabemos por qué (probablemente el `image PUT failed (524)` de anoche). `/generate` sí muestra el motivo. Cambio: el `catch (e)` externo de `/compose` devuelve `json({ error: e && e.message ? e.message : 'Scene service unavailable' }, 502)` — sin la frase «Submission may have been accepted» (el único caso en que el envío pudo aceptarse es que el `fetch` a fal lance tras enviar, y para eso el cliente ya tiene su propio guardián `uncertain` cuando la respuesta no llega). Prueba en `tests/composer-worker.mjs`: presign que devuelve 401 `{detail:'Invalid credentials'}` → `/compose` responde 502 con `error` que contiene `upload-url 401`; `PUT` que devuelve 524 → 502 con `image PUT failed (524)`.
